defmodule SymphonyElixir.LiveE2ESupport do
  @moduledoc false

  import ExUnit.Assertions

  alias SymphonyElixir.{SSH, TestSupport}
  ...
  @spec receive_runtime_info!(String.t()) :: map()
  def receive_runtime_info!(issue_id) when is_binary(issue_id) do
    receive do
  ...
  @spec read_worker_result!(map(), String.t()) :: String.t()
  def read_worker_result!(%{worker_host: nil, workspace_path: workspace_path}, result_file)
      when is_binary(workspace_path) and is_binary(result_file) do
    File.read!(Path.join(workspace_path, result_file))
  end

  def read_worker_result!(%{worker_host: worker_host, workspace_path: workspace_path}, result_file)
    ...
    case SSH.run(worker_host, "cat #{shell_escape(remote_result_path)}", stderr_to_stdout: true) do
      ...
      {:ok, {output, status}} ->
        flunk(
          "failed to read remote result from #{worker_host}:#{remote_result_path} " <>
            "(status #{status}): #{inspect(output)}"
        )
  ...
  @spec live_worker_setup!(:local | :ssh, String.t(), String.t()) :: map()
  def live_worker_setup!(:local, _run_id, test_root) when is_binary(test_root) do
    %{
      cleanup: fn -> :ok end,
  ...
  def live_worker_setup!(:ssh, run_id, test_root) when is_binary(run_id) and is_binary(test_root) do
  ...
  @spec cleanup_live_worker_setup(map()) :: :ok
  def cleanup_live_worker_setup(%{cleanup: cleanup}) when is_function(cleanup, 0) do
    cleanup.()
  end

  def cleanup_live_worker_setup(_worker_setup), do: :ok

  @spec restart_orchestrator_if_needed() :: :ok
  def restart_orchestrator_if_needed do
  ...
  defp live_docker_worker_setup!(run_id, test_root) when is_binary(run_id) and is_binary(test_root) do
    ...
    worker_host = live_docker_worker_host()
    worker_hosts = Enum.map(worker_ports, &"#{worker_host}:#{&1}")
  ...
  defp live_docker_worker_host do
    System.get_env("SYMPHONY_LIVE_DOCKER_WORKER_HOST", "localhost")
    |> String.trim()
    |> case do
      "" -> "localhost"
      host -> host
